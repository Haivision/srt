...
| [`SRT_EINVALMSGAPI`](#srt_einvalmsgapi)       | Incorrect API usage in **message mode**:<br/>**live mode**: the receive buffer is too small to hold a maximum-sized payload. Ensure the buffer is at least [`SRTO_PAYLOADSIZE`](API-socket-options.md#SRTO_PAYLOADSIZE) bytes (default 1316). |
