...
| [pktRcvFilterRecoveredTotal](#pktRcvFilterRecoveredTotal)     | accumulated       | packets             | -                    | ✓                      | int32_t   |
| [pktRcvFilterUnrecoveredTotal](#pktRcvFilterUnrecoveredTotal) | accumulated       | packets             | -                    | ✓                      | int32_t   |
...
DATA packets recovered by the packet filter ([pktRcvFilterRecoveredTotal](#pktRcvFilterRecoveredTotal)) are taken into account if the `SRTO_PACKETFILTER` socket option is enabled (refer to [API.md](API.md)). Do not mix up with the control packets received by the packet filter ([pktRcvFilterExtraTotal](#pktRcvFilterExtraTotal)).
...
Same as `pktRcvFilterRecoveredTotal`, but for a specified interval.

Introduced in v1.4.0. Refer to [SRT Packet Filtering & FEC](packet-filtering-and-fec.md).

### pktRcvFilterLoss

Same as `pktRcvFilterUnrecoveredTotal`, but for a specified interval.
