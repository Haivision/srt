...
void FECFilterBuiltin::ClipPacket(Group& g, const CPacket& pkt)
{
    ...
    HLOGC(pflog.Debug, log << "FEC DATA PKT CLIP: "
            << "FLAGS=" << fmt::ffmt<unsigned>(kflg, fmt::hex)
            << " LENGTH[ne]=" << fmt::ffmt(length_net, fmt::hex)
            << " TS[he]=" << fmt::ffmt(timestamp_hw, fmt::hex)
            << " CLIP STATE: FLAGS=" << fmt::ffmt<unsigned>(g.flag_clip, fmt::hex)
            << " LENGTH[ne]=" << fmt::ffmt(g.length_clip, fmt::hex)
            << " TS[he]=" << fmt::ffmt(g.timestamp_clip, fmt::hex)
            << " PL4=" << fmt::ffmt(*(uint32_t*)&g.payload_clip[0], fmt::hex));
}
...
void FECFilterBuiltin::ClipControlPacket(Group& g, const CPacket& pkt)
{
    ...
    HLOGC(pflog.Debug, log << "FEC/CTL CLIP: "
            << "FLAGS=" << fmt::ffmt<unsigned>(*flag_clip, fmt::hex)
            << " LENGTH[ne]=" << fmt::ffmt(*length_clip, fmt::hex)
            << " TS[he]=" << fmt::ffmt(timestamp_hw, fmt::hex)
            << " CLIP STATE: FLAGS=" << fmt::ffmt<unsigned>(g.flag_clip, fmt::hex)
            << " LENGTH[ne]=" << fmt::ffmt(g.length_clip, fmt::hex)
            << " TS[he]=" << fmt::ffmt(g.timestamp_clip, fmt::hex)
...
void FECFilterBuiltin::PackControl(const Group& g, signed char index, SrtPacket& pkt, int32_t seq)
{
    ...
    HLOGC(pflog.Debug, log << "FEC: PackControl: hdr("
            ...
            << int(index) << " LENGTH[ne]=" << fmt::ffmt(g.length_clip, fmt::hex)
            << " FLAGS=" << fmt::ffmt<unsigned>(g.flag_clip, fmt::hex)
            << " TS=" << fmt::ffmt(g.timestamp_clip, fmt::hex)
            << " PL(" << g.payload_clip.size() << ")[0-4]="
            << fmt::ffmt(*(uint32_t*)&g.payload_clip[0], fmt::hex));
}
