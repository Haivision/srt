    ...
    CRYSPR_AESCTX *aes_key)     /* CRYpto Service PRovider AES Key context */
{
    ...
    case HCRYPT_CTX_MODE_AESCTR:
        ...
        break;
    default:
        HCRYPT_LOG(LOG_ERR, "invalid cipher type (%d). Expected: [%d..%d]\n",
                   cipher_type, HCRYPT_CTX_MODE_AESECB, HCRYPT_CTX_MODE_AESCTR);
        return(-1);
    }
    switch(kstr_len) {
    case 128/8:
...
int crysprOpenSSL_EVP_AES_EcbCipher(
    ...
    size_t *outlen_p)             /* in/out dst len */
{
    ...
    if(!EVP_CIPHER_CTX_set_padding(aes_key, 0)){
        HCRYPT_LOG(LOG_ERR, "%s\n", "EVP_CIPHER_CTX_set_padding(%p) failed",
                   aes_key);
        return -1;
    }
    ...
    if (0 == EVP_CipherFinal_ex(aes_key, &out_txt[c_len], &f_len)) {
        ...
        char szErrBuf[256];
        HCRYPT_LOG(LOG_ERR, "EVP_CipherFinal_ex(ctx,&out[%d],%d)) failed: %s\n",
                   c_len, f_len, ERR_error_string(ERR_get_error(), szErrBuf));
...
int crysprOpenSSL_EVP_AES_CtrCipher(
    bool bEncrypt,              /* true:encrypt, false:decrypt */
    CRYSPR_AESCTX *aes_key,     /* CRYpto Service PRovider AES Key context */
    unsigned char *iv,          /* iv */
...
*/
int crysprOpenSSL_EVP_KmPbkdf2(
    CRYSPR_cb *cryspr_cb,
...
int crysprOpenSSL_EVP_KmWrap(CRYSPR_cb *cryspr_cb,
		unsigned char *wrap,
...
int crysprOpenSSL_EVP_KmUnwrap(
        CRYSPR_cb *cryspr_cb,
