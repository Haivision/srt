...
$Env:VS_VERSION = $VS_VERSION
 # build SRT
if ( $VS_VERSION -eq '2019' -and $DEVENV_PLATFORM -eq 'Win32' ) { $CMAKE_GENERATOR = 'Visual Studio 16 2019'; $CMAKE_ARCH = "-A Win64" }
if ( $VS_VERSION -eq '2019' -and $DEVENV_PLATFORM -eq 'x64' ) { $CMAKE_GENERATOR = 'Visual Studio 16 2019'; $CMAKE_ARCH = "-A x86" }
...
cmake ../ -G"$CMAKE_GENERATOR" $CMAKE_ARCH -DCMAKE_BUILD_TYPE=$CONFIGURATION -DENABLE_STDCXX_SYNC=ON -DENABLE_UNITTESTS="$ENABLE_UNITTESTS" -DOPENSSL_USE_STATIC_LIBS="$ENABLE_STATIC_LINK_SSL"
